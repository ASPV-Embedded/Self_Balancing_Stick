...
static HAL_StatusTypeDef EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data);
...
HAL_StatusTypeDef EE_Init(void)
{
	...
	case ERASED:
		...
		{
			/* Erase Page0 */
			EraseInit.Sector = PAGE0_ID;
			FlashStatus = HAL_FLASHEx_Erase(&EraseInit, &SectorError);
